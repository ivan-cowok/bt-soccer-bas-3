...
class CBAM(nn.Module):
    ...
    Applied on HR spatial feature maps (B*T, C, H', W') before global pooling.
    Channel attention learns "which features matter"; spatial attention learns "where".
    """
    ...
    def __init__(self, channels, reduction=16, spatial_kernel=7):
        ...
        self.sp_conv = nn.Conv2d(2, 1, kernel_size=spatial_kernel,
    ...
    def forward(self, x):
        ...
        ch_w  = torch.sigmoid(self.ch_mlp(avg_c) + self.ch_mlp(max_c))
        x = x * ch_w.view(B, C, 1, 1)

        # Spatial attention
        avg_s = x.mean(dim=1, keepdim=True)           # (B, 1, H, W)
        max_s = x.max(dim=1, keepdim=True)[0]          # (B, 1, H, W)
        sp_w  = torch.sigmoid(self.sp_conv(torch.cat([avg_s, max_s], dim=1)))
        x = x * sp_w
...
class ROISelector(nn.Module):
    """
    Training-free saliency RoI. Optional improvements for hard domains (e.g. low-budget video):
    - channel_reduce: 'mean' | 'max' | 'mean_max' — sharper peaks than mean-only
    - spatial_increase: upsampling before peak search (higher = finer grid, more compute)
    - size_step: spacing of candidate window sizes in pixels (smaller = more scales)
    """
    ...
    def __init__(
        ...
        assert channel_reduce in ('mean', 'max', 'mean_max'), channel_reduce
    ...
    @staticmethod
    def _minmax_per_frame(x_bt_hw):
        """x: (B, T, 1, H, W) -> same shape, per-(B,T) min-max to [0,1]."""
        B, T, _, H, W = x_bt_hw.shape
        v = x_bt_hw.view(B, T, -1)
        mn = v.min(dim=-1, keepdim=True)[0]
        mx = v.max(dim=-1, keepdim=True)[0]
        v = (v - mn) / (mx - mn + 1e-5)
        return v.view(B, T, 1, H, W)
    ...
    def forward(self, x):
        ...
        # ---- Step 1: Reduce channel dimension (saliency proxy) ----
        if self.channel_reduce == 'mean':
            s = x.mean(dim=2, keepdim=True)
        elif self.channel_reduce == 'max':
            s = x.max(dim=2, keepdim=True)[0]
        else:
            m = x.mean(dim=2, keepdim=True)
            mx = x.max(dim=2, keepdim=True)[0]
            s = torch.maximum(m, mx)

        # Min-max normalize per frame (fixed threshold semantics)
        x = self._minmax_per_frame(s)
        ...
        # Initialize outputs (-1 = not yet assigned; 0 is a valid linear index)
        center = torch.full((B, 1, T), -1.0, device=x.device)
        ...
        # If no scale passed threshold: use largest window (already below) and saliency argmax
        # on the upsampled probability map (not LR H*W//2 — that was inconsistent).
        flat = x.view(B, 1, T, -1)
        saliency_argmax = flat.argmax(dim=-1).float()  # (B, 1, T)
        unset = center < 0
        center = torch.where(unset, saliency_argmax, center)
        size_h[size_h == 0] = self.sizes[0][-1]
